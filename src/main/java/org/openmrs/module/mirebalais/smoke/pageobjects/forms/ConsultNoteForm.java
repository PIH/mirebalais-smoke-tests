...
public class ConsultNoteForm extends BaseHtmlForm {
    ...
    public void fillFormWithDeath(String primaryDiagnosis) throws Exception {
        ...
        Thread.sleep(30000); // give the dx widget time to be hidden, for some reason there seems to be an error at times,  where the widget blocks entering the date of death
        chooseDisposition(DEATH);
        wait30seconds.until(visibilityOfElementLocated(By.cssSelector("#dateOfDeath input[type=text]")));
